...
public class SmileyFace extends ApplicationAdapter {
    ...
    static final float LEFT_EYE_CENTER_X = FACE_CENTER_X - EYE_OFFSET;
    static final float LEFT_EYE_CENTER_Y = FACE_CENTER_Y + EYE_OFFSET;
    static final float RIGHT_EYE_CENTER_X = FACE_CENTER_X + EYE_OFFSET;
    static final float RIGHT_EYE_CENTER_Y = FACE_CENTER_Y + EYE_OFFSET;
    static final float MOUTH_X = 9f;
    static final float MOUTH_Y = 10f;
    ...
    private void drawSmileyFace(ShapeRenderer renderer) {
        ...
        renderer.circle(LEFT_EYE_CENTER_X, LEFT_EYE_CENTER_Y, EYE_RADIUS, EYE_SEGMENTS);
        renderer.circle(RIGHT_EYE_CENTER_X, RIGHT_EYE_CENTER_Y, EYE_RADIUS, EYE_SEGMENTS);

        // TODO: Draw a black arc for the mouth (Hint: MOUTH_OUTER_RADIUS)
        renderer.arc(FACE_CENTER_X, FACE_CENTER_Y, MOUTH_OUTER_RADIUS, MOUTH_START_ANGLE, MOUTH_DEGREES,
                MOUTH_SEGMENTS);

        // TODO: Draw a yellow arc to make the mouth actually look like a mouth (Hint: MOUTH_INNER_RADIUS)
        renderer.setColor(Color.YELLOW);
        renderer.arc(FACE_CENTER_X, FACE_CENTER_Y,
                MOUTH_INNER_RADIUS, MOUTH_START_ANGLE, MOUTH_DEGREES, MOUTH_SEGMENTS);
    }
}
